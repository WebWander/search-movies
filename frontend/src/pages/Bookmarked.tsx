...
const Bookmarked: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBookmarkedMovies = async () => {
      try {
        const response = await axios.get(`https://search-movies-8a15.onrender.com/api/bookmarks`);
  ...
  const removeBookmark = async (movieId: string) => {
    try {
      const response = await axios.post(`https://search-movies-8a15.onrender.com/api/bookmarks/remove`, { movie: movieId });
